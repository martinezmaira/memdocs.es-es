---
title: Datos de diagnóstico y uso en la versión 2002
titleSuffix: Configuration Manager
description: Obtenga más información sobre los datos específicos de cada nivel que recopila Configuration Manager en la versión 2002.
ms.date: 04/01/2020
...
En el caso de la versión 2002 de Configuration Manager, este nivel incluye los siguientes datos:

- Estadísticas sobre las conexiones de la consola de Configuration Manager: versión de SO, idioma, SKU y arquitectura; memoria del sistema, número de procesadores lógicos, identificador de sitio de conexión, versiones instaladas de .NET, paquetes de idioma de la consola y nivel de autenticación compatible.
...
- ***[Actualizado]*** Datos básicos de la jerarquía del sitio de Configuration Manager: lista de sitios, tipo, versión, estado, número de clientes, zona horaria y estado de mantenimiento
...
- ***[Quitado]*** Recuento de clientes de Windows 10 que usan Windows Update para empresas  
...
- ***[Quitado]*** Recuento de las implementaciones por fases creadas por tipo  
...
- Estado del servicio de administración

- Hash de los atributos de sitio clave (identificador del sitio, identificador del agente de SQL y la clave de intercambio de sitio)

- Recuento de instalaciones de Microsoft Edge

- ***[Movido]*** Recuento de servicios y aplicaciones de Azure Active Directory conectados a Configuration Manager

- ***[Nuevo]*** Información sobre el estado del sitio

- ***[Nuevo]*** Ubicaciones de bloqueos de la consola de Configuration Manager

- ***[Nuevo]*** Estadísticas de uso de la consola de Configuration Manager

- ***[Nuevo]*** Acciones de conexión y desconexión a la nube

- ***[Nuevo]*** Estado de la última sincronización con el servicio en la nube de Intune

- ***[Nuevo]*** Recuento de errores del servicio de administración

- ***[Nuevo]*** Uso del token de registro masivo

- ***[Nuevo]*** Recuento de clientes mediante el explorador predeterminado y preferido
...
- Recuento y tamaño de las recopilaciones piloto de Office Pro Plus

- ***[Nuevo]*** Número de dispositivos de Office Pro Plus que envían datos del estado de Office
...
- ***[Nuevo]*** Uso del asistente para la incorporación de servicios en la nube
...
- Recuento de recopilaciones sincronizadas con Azure Active Directory
...
- Recuento de carpetas

- ***[Nuevo]*** Información de rendimiento de la consola

- ***[Nuevo]*** 25 acciones más comunes, asistentes, hojas de propiedades y nodos de árbol a los que se puede acceder desde la consola
...
- ***[Nuevo]*** Recuento de subredes excluidas de la caché del mismo nivel
...
- ***[Nuevo]*** Estadísticas acumuladas para las directivas de Protección contra amenazas avanzada de Microsoft Defender
...
- Recuento de los dispositivos móviles administrados por Configuration Manager, así como la forma en que se han inscrito (masiva, basada en usuarios)  
...
- Recuento de identificadores de hardware duplicados (dirección MAC y GUID de SMBIOS) excluidos del registro de cliente y el entorno PXE

- Recuento de secuencias de tareas por tipo (secuencia de tareas genérica o implementación del SO)

- Recuento de paquetes con configuración del contenido de la caché previa
...
- ***[Quitado]*** Configuración de las actualizaciones rápidas de Windows 10  
...
- ***[Quitado]*** Recuento de grupos de actualizaciones y asignaciones  

- ***[Quitado]*** Recuento de paquetes de actualización y del número mínimo, máximo y promedio de puntos de distribución de destino con paquetes  

- ***[Quitado]*** Recuento de actualizaciones que se crean e implementan con System Center Updates Publisher  

- ***[Quitado]*** Recuento de directivas creadas e implementadas de Windows Update para empresas  

- Estadísticas acumuladas de configuraciones de Windows Update para empresas  

- ***[Quitado]*** Número de reglas de implementación automática que están asociadas a la sincronización  

- ***[Quitado]*** Número de reglas de implementación automática que crean actualizaciones nuevas o agregan otras a un grupo existente  
...
- ***[Quitado]*** Número total o promedio de recopilaciones que tienen implementaciones de actualización de software y el número máximo o promedio de actualizaciones implementadas  
...
- Lista de las suscripciones a los catálogos de actualización de software de terceros

- Uso de la configuración de mantenimiento de WSUS

- ***[Nuevo]*** Uso de un grupo de orquestación

- ***[Nuevo]*** Opciones de configuración de reserva de Windows Update
...
- Estadísticas de rendimiento acumuladas de las llamadas hechas a los puntos de conexión del Centro de software en el punto de administración

- Estado y configuración de las tareas de mantenimiento de SQL

- ***[Nuevo]*** Estado de las solicitudes de reinicialización recientes
...
- Estadísticas acumuladas sobre el uso de Análisis de escritorio

- Uso de la herramienta de migración de Azure

- ***[Nuevo]*** Recuento de clientes con uso del explorador

## <a name="level-3---full"></a><a name="bkmk_level3"></a> Nivel 3: Completo

En el caso de la versión 2002 de Configuration Manager, este nivel incluye los siguientes datos:

- ***[Quitado]*** Información de la programación de evaluación de reglas de implementación automática  
...
- ***[Quitado]*** Número mínimo, máximo y promedio de horas desde la última búsqueda de actualizaciones de software  

- ***[Quitado]*** Número mínimo, máximo y promedio de clientes inactivos en recopilaciones de implementación de actualizaciones de software  
...
- ***[Quitado]*** Recuento de grupos que tienen actualizaciones de software expiradas  
...
- Lista de las extensiones de la consola de Configuration Manager
